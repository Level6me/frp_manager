...
```bash
cd /Users/user/.openclaw/workspace/frp-web-manager

# 如果还没添加远程仓库
git remote add origin http://gogs.example.com/claw/frp_manager.git
...
git remote set-url origin git@gogs.example.com:claw/frp_manager.git
```
...
# 在目标服务器上执行
cd /opt
sudo git clone http://gogs.example.com/claw/frp_manager.git
cd frp-web-manager
