    ...
    return fetch('/api/tunnels')
        ...
        .then(d => {
            ...
            if(d.success) {
                ...
                document.getElementById('metricActiveConns').textContent = d.total_active_conns;
                ...
                document.getElementById('metricRunningTunnels').textContent = running;
                document.getElementById('metricErrorTunnels').textContent = error;
            } else {
            ...
            if(d.proxies.length === 0) {
                ...
                list.innerHTML = d.proxies.map(function(p, i) {
                    ...
                    if (latestTunnels && latestTunnels[p.name]) {
                        ...
                        if (t.status === 'running') {
                            statusHtml = `<span style="font-size:12px;color:var(--apple-green);margin-left:8px;font-weight:600">🟢 在线 (活动连接: ${t.active_conns})</span>`;
                        } else {
